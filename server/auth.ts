...
export function setupAuth(app: Express) {
  const sessionSettings: session.SessionOptions = {
    secret: process.env.SESSION_SECRET || "scambane-security-session",
    resave: false,
  ...
  app.post("/api/register", async (req, res, next) => {
    try {
      ...
      const existingPhone = await storage.getUserByPhoneNumber(req.body.phoneNumber);
      if (existingPhone) {
  ...
  app.post("/api/request-otp", async (req, res) => {
    try {
      ...
      // For demo purpose, just return the OTP in development
  ...
  app.post("/api/verify-otp", async (req, res, next) => {
    try {
      ...
      if (!user) {
        // Auto-create user for phone-based login
